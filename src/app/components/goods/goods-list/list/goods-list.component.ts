...
export class GoodsListComponent implements OnInit, OnChanges {
  ...
  private paginate(): void {
    ...
    this.pageArray = Array(this.totalPageCount < displayedPageCount ? this.totalPageCount : displayedPageCount).fill(this.currentPage).map((e, i) => {
      if (e < Math.ceil(displayedPageCount / 2) + 1) {
        ...
        if (e < this.totalPageCount - Math.floor(displayedPageCount / 2)) {
  ...
  public doUpdate() {
    this.goodsService.updateStatuses(this.goodsList.filter(item => {
        return item.changed;
      }
    )).subscribe(res => {
  ...
  private goToStorageView(goods) {
    if (this.isEditable) {
      ...
      this.router.navigate(['../typespace', goods.goods.id, 'warehouse', this.warehouseId, 'put'], {relativeTo: this.route});
    }
  }
